...
public class GameBoard {
	...
	public void render(Graphics2D g){
		...
		g.drawString("Meilleur: "+ highScore, Game.WIDTH - DrawUtils.getMessageWidth("Meilleur: " + highScore, scoreFont, g)-20, 40);
	}
	...
	private void resetPosition(Tile current, int row, int col){
		...
		int ditY = current.getY() - y;
		
		if(Math.abs(distX)< Tile.SLIDE_SPEED){
			current.setX(current.getX() - distX);
		}
		
		if(Math.abs(ditY) < Tile.SLIDE_SPEED){
			current.setY(current.getY() - ditY);//dist* 
		}
